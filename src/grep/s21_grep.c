...
FILE *open_file(int argc, char *argv[], int k, int count_flags,
                int arg_number_regulars);

int one_file(int argc, int number_regulars, char *argv, char *op_flags,
...
void outputMatches(int *match_count, int *schet, int count_flags,
                   char *op_flags, int argc, int *line_number, char *argv,
                   regex_t reg, char *row, int arg_number_regulars);
...
FILE *open_file(int argc, char *argv[], int k, int count_flags,
                int arg_number_regulars) {
  FILE *fp = stdin;
  if (argc - 1 != arg_number_regulars + count_flags) {
    fp = fopen(argv[k], "r");
  }
  return fp;
}
...
void outputMatches(int *match_count, int *schet, int count_flags,
  ...
  *match_count += 1;
  *schet = 0;
  if (argc - 2 > arg_number_regulars + count_flags &&
...
void print_file(regex_t *regex, int number_regulars, int count_flags,
  ...
  for (int i = 0; i < number_regulars && end_while != -1; i++) {
    ...
    if ((res == 0 && v == NULL) || (res != 0 && v != NULL)) {
      outputMatches(match_count, schet, count_flags, op_flags, argc,
                    &line_number, argv, regex[i], row, arg_number_regulars);
      i = number_regulars;
    }
  }
}
...
int read_and_print(int argc, char **argv, char regulars[100][100],
  ...
  if (arg_number_regulars + count_flags + 1 == argc) f = 0;
  for (int k = arg_number_regulars + count_flags + f; flag == OK && k < argc;
       k++) {
    fp = open_file(argc, argv, k, count_flags, arg_number_regulars);
    if (fp != NULL) {
      flag = one_file(argc, number_regulars, argv[k], op_flags, count_flags,
                      regex, fp, arg_number_regulars);
...
int read_file_regulars(char *filename, char regulars[100][100],
                       int *number_regulars) {
  FILE *f = NULL;
  int flag = OK;
...
int options_and_regulars(int argc, char **argv, char regulars[100][100],
  ...
  for (int i = 1; i < argc && flag == OK; i++) {
    ...
    if (argum[0] == '-') {
      ...
      *count_flags += 1;
      if (flag_e > 0 && flag_f < 0 && (int)strlen(argv[i]) - 1 > flag_e &&
          (int)strlen(argv[i]) > 0) {
        strcpy(regulars[*number_regulars], argv[i] + flag_e + 1);
        *number_regulars += 1;
        *arg_number_regulars += 1;
        *count_flags -= 1;
        flag_e = -1;
      }
      if (flag_f > -1 && (int)strlen(argv[i]) - 1 > flag_f &&
          (int)strlen(argv[i]) > 0) {
        flag =
            read_file_regulars(argv[i] + flag_f + 1, regulars, number_regulars);
        flag_f = -1;
        *arg_number_regulars += 1;
        *count_flags -= 1;
      }
    } else if (flag_f > -1) {
      flag = read_file_regulars(argv[i], regulars, number_regulars);
      flag_f = -1;
    } else if (flag_e > -1 && flag_f < 0) {
